...
Veamos ahora cómo solicitaremos tablas de frecuencias, porcentajes y estadísticos simples con SegmentaNet y R.
...
Para este primer script, indicaremos que usamos la fuente de datos (dataframe) cargado en el análisis. En SegmentaNet siempre se llama `data` a este dataframe cargada en el análisis. Si estamos en RSTudio, el dataframe tendrá el nombre que le hayas indicado en la carga. Redactamos pues nuestro script, donde identificamos el dataframe, el campo `P31` del cual vamos a calcular el número de casos:
...
```{r fig.cap='Frecuencias de P31, moviendo Total', out.width='80%', fig.asp=.75, fig.align='center', echo=TRUE}
data %>% 
  tab_cells(P31) %>% 
...
```{r fig.cap='Frecuencias de P18 (múltiple)', out.width='80%', fig.asp=.75, fig.align='center', echo=TRUE}
data %>% 
...
```{r fig.cap='Frecuencias y porcenatjes de P18 (múltiple)', out.width='80%', fig.asp=.75, fig.align='center', echo=TRUE}
data %>% 
...
```{r fig.cap='Frecuencias y porcentajes de P18 (múltiple)', out.width='80%', fig.asp=.75, fig.align='center', echo=TRUE}
data %>% 
  tab_cells(mdset_f(P18C)) %>%  # o tab_cells(mdset(P18C01 %to% P18C08))
  tab_stat_cases(label ="Casos") %>% 
  tab_stat_cpct(label="% casos") %>% 
